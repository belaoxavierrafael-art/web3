...
export function SheetContent({ children, className }) {
  return (
    <Dialog.Portal>
      <Dialog.Overlay className="sheet-overlay fixed inset-0 z-40 bg-ink/70 backdrop-blur-sm" />
      <Dialog.Content className={cn('sheet-panel fixed inset-y-0 right-0 z-50 flex w-[min(24rem,90vw)] flex-col border-l-[3px] border-gold bg-[radial-gradient(circle_at_top,_rgba(244,203,93,0.18),_transparent_42%),_hsl(var(--paper))] p-5 text-ink shadow-[0_30px_80px_rgba(0,0,0,0.35)] outline-none backdrop-blur-xl', className)}>
        {children}
